...
| `/set reveal on\|off` | 逐字跟读开关（默认开；`off` = 一次性全输出） |
...
> 这一版是全量功能自检（`_learn/fullcheck.py`，110+ 项）查出来的问题修复。

- **修正我之前一个错误结论：分句流式渲染并非逐样本无损。**
  v1.07 加"边渲染边播放"时，我的验证结论是
  「分句拼接与整段渲染**逐样本一致**」。但那批测试用的是**手写的恒定音符**
  （音高整数、时长全部一样）。换成真实语音重测后：**只要音符时长有变化，
  各块开头就渲染得不一致** —— 波形差异 RMS 占信号的 38%~58%。
  试过给每块补前文上下文（1/2/3 个音，甚至把前面全部内容都当上下文），
  差异都降不到 0 —— 说明渲染器对某个音的输出取决于**整个 part**，
  不只是前文，所以这条路走不通。
  - **但差异是听不出来的**：20ms 短时能量包络的相关度 **0.9998**、
    平均偏差 **0.7%**，也就是响度起伏、节奏、咬字时间点都对得上，
    差的只是相位/时序上的微移。波形差 ≠ 听感差。
  - 默认仍然改回**整段一次渲染**（逐样本无损，最保守）；
    想要首句更快出声用 `/set stream on`。两个都在，你自己权衡。
  - 教训记下来：**用逐样本差当质量指标是错的**，那衡量的是相位，
    不是听感。以后这类判断要看能量包络或做 A/B 试听。
- **配置文件容错**：`config.json` 损坏时以前直接抛 `FileNotFoundError`
  导致程序起不来。现在会把**坏文件备份成 `.bad`**（里面可能有你的 API Key），
  退回默认配置继续启动；缺失的键自动补默认值，老配置能直接升级。
- **配置原子写入**：`save_config` / `save_characters` 改成
  "先写临时文件再 `os.replace`"。以前直接覆盖，写到一半断电或被杀进程
  就会留下半截 JSON，下次启动就读不了。
- **`audio_filename` 剔除孤立代理项**：以前会生成一个没法落盘的文件名。

### v1.10

- **渲染失败时给出可排查的信息**：以前界面只显示 str(e)，像
  JSONDecodeError 这种只显示一句「尾部有多余内容」，既看不出是哪一句、
  哪个文件，也看不到调用栈。现在：
  - 界面显示 异常类型 + 消息 + 出错位置(file:line)
  - **完整堆栈写进 <数据目录>\logs\error.log**，界面上会提示日志路径
  - 渲染器返回非零时，把**渲染器路径 / 声库路径 / 输出路径 / 控制台输出**
    一起附上，一眼能看出去哪找问题
- **修掉「渲染随机失败」**：v1.09 为了彻底干掉多余的控制台窗口用了
  FreeConsole()。它确实把窗口销毁了，但**指向控制台的标准句柄会一起失效**——
  之后 subprocess 建渲染进程时复制这些句柄就报
  OSError [WinError 6] 句柄无效，表现为**渲染时好时坏**。
  实测三种改法（只重定向 fd 1/2、再接 fd 0、再显式 SetStdHandle）都会翻车，
  因为句柄数值会不会被复用取决于环境。
  **现在完全不用 FreeConsole**：只 ShowWindow 藏起来，再加一个
  **看门狗线程每秒检查一次，冒头就再藏**。完全不动标准句柄，
  既不炸渲染，窗口也留不住。顺带把「弹 Tk 对话框时控制台还杵在旁边」
  也修了（选界面/首次设置之前先藏，选 CLI 再还原）。
  回归测试：控制台处理后连渲 10 次，10/10 成功。
- **修掉「台词先中文、突然变日文」**：显示语言与朗读语言不同时，
  先亮开头那段用的是**译文**，而跟读推进时误用了**朗读原文**——
  于是中文先冒出来，音频一开播又被日文整行覆盖，看着像突然变回日文。
  现在这一行自始至终用同一份文本（有译文就用译文）。
- **逐字跟读不再开局一片空白**：以前要等「接口 + 首句渲染 + 2.5 秒预缓冲」
  走完才看到第一个字，比一次性全输出还慢。现在**回复一到就先亮出开头
  20 个字**（speak.TEXT_HEAD_CHARS），后面的字再跟着语音走。
- **逐字跟读的字数改成只增不减**：音频刚开播时按公式算出的是「提前 10 字」
  也就是 10 个字，比"先亮开头"的 20 个字**少** —— 直接写回去会让已经显示的
  文字**缩回去**（表现为"先显示一小段、又缩回 10 个字"）。现在字数只允许
  变多，不会再回缩。验证脚本 `_learn/_verify_reveal_text.py` 会断言这一点。
- **修掉滚动时下方留白**：以前是按"末尾那些行"算出一个固定行数 `fit`，
  然后**滚到哪都画 fit 行**。但行高不一样（横幅是小字号、正文是大字号），
  滚上去时窗口里混进矮行，`fit` 行占不满一屏，视口下方就空出一块
  —— 看起来像"往上滚，下面的文字消失了"。
  现在改成**从下往上按实际行高填满**，并把行高缓存起来。
  顺带修了缓存键漏掉宽度的问题（窗口缩放后行高会算错）。
  验证脚本 `_learn/_verify_scroll.py` 会逐格检查填充率。
- **音符 JSON 加自检**：调用渲染器之前先把刚写出的 JSON 读回来解析一遍。
  渲染器偶尔报 invalid after a single JSON value. Expected end of data.
  （JSON 尾部有多余内容），但那是 .NET 侧的说法，看不出是谁写坏的。
  现在如果这份 JSON 本身有问题，当场报出音符数和文件大小，
  不会再让人对着渲染器的报错猜。

### v1.09

- **彻底修掉那个多余的控制台窗口**：GUI 模式下只把控制台 ShowWindow(SW_HIDE)
  藏起来是不够的 —— 远程桌面连接、切换用户等情况会让它再冒出来
  （你已经遇到两次，一次是报错弹窗、一次是空白窗口）。现在改成
  先把输出接到日志、再 FreeConsole() **把控制台直接销毁**，
  窗口不会再回来。OS 层的 fd 1/2 也一并重定向，
  原生库（onnxruntime / voicevox）的输出同样进日志，不会写到无效句柄上。
- **逐字跟读做成开关**：/set reveal on | off
  - on（默认）：播到哪显示到哪，文字比语音提前 10 个字
  - off：恢复成原来的一次性全输出
  设置面板里叫「逐字跟读」，改完立即生效。
